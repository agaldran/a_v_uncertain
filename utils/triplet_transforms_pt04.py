...
class Compose(object):
    ...
    def __call__(self, img, target=None, mask=None):
        ...
        if target is not None and mask is not None:
            for t in self.transforms:
                img, target, mask = t(img, target, mask)
...
class ToTensor(object):
    ...
    def __call__(self, pic, pic2=None, pic3 = None):
        """
        Args:
            pic (PIL Image or numpy.ndarray): Image to be converted to tensor.
            pic2 (PIL Image): (optional) Second image to be converted also.
            pic3 (PIL Image): (optional) Thrid image to be converted also.

        Returns:
            Tensor(s): Converted image(s).
        """
        if pic2 is not None and pic3 is None:
            return F.to_tensor(pic), F.to_tensor(pic2)
        if pic2 is not None and pic3 is not None:
            return F.to_tensor(pic), F.to_tensor(pic2), F.to_tensor(pic3)
...
class ToPILImage(object):
    ...
    def __call__(self, pic, pic2=None, pic3 = None):
        """
        Args:
            pic (Tensor or numpy.ndarray): Image(s) to be converted to PIL Image.
        ...
        if pic2 is not None and pic3 is not None:
            return F.to_pil_image(pic), F.to_pil_image(pic2), F.to_pil_image(pic3)
...
class Resize(object):
    ...
    def __call__(self, img, target = None, mask=None):
        ...
        Args:
            ...
            mask (PIL Image): (optional) Mask to be scaled
        ...
        if target is not None and mask is not None:
            return F.resize(img, self.size, self.interpolation), \
                   ...
                   F.resize(mask, self.size, self.interpolation_mask)
        return F.resize(img, self.size, self.interpolation)
...
class CenterCrop(object):
    ...
    def __call__(self, img, target=None, mask = None):
        ...
        Args:
            ...
            mask (PIL Image): (optional) Mask to be cropped
        ...
        if target is not None and mask is not None:
            return F.center_crop(img, self.size), F.center_crop(target, self.size), F.center_crop(mask, self.size)
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, target=None, mask=None):
        ...
        if random.random() < self.p:
            ...
            elif target is not None and mask is not None:
                return F.hflip(img), F.hflip(target), F.hflip(mask)
            else:
                return F.hflip(img)
    ...
    def __call__(self, img, target=None, mask=None):
        ...
        if random.random() < self.p:
            if target is not None and mask is None:
                ...
                return F.vflip(img), F.vflip(target), F.vflip(mask)
            else:
                return F.vflip(img)
...
class RandomResizedCrop(object):
    ...
    def __call__(self, img, target = None, mask=None):
        ...
        if target is not None and mask is not None:
            ...
            return F.resized_crop(img, i, j, h, w, self.size, self.interpolation), \
                   F.resized_crop(target, i, j, h, w, self.size, self.interpolation_tg), \
                   F.resized_crop(mask, i, j, h, w, self.size, self.interpolation_mask)
...
class FiveCrop(object):
    ...
    def __call__(self, img, target=None, mask = None):
        ...
        if target is not None and mask is not None:
            return F.five_crop(img, self.size), F.five_crop(target, self.size), F.five_crop(mask, self.size)
...
class TenCrop(object):
    ...
    def __call__(self, img, target = None, mask = None):
        ...
        if target is not None and mask is not None:
            return F.ten_crop(img, self.size), F.ten_crop(target, self.size), F.ten_crop(mask, self.size)
...
class ColorJitter(object):
    ...
    def get_params(brightness, contrast, saturation, hue):
        ...
        if brightness > 0:
            ...
            # brightness_factor = random.uniform(max(0, 1 - brightness), 1.0)
    ...
    def __call__(self, img, target = None, mask = None):
        ...
        if target is not None and mask is not None:
            return transform(img), target, mask
...
class RandomRotation(object):
    ...
    def __init__(self, degrees, resample=False, resample_tg=False, resample_mask=False, expand=False, center=None):
    ...
    def __call__(self, img, target=None, mask=None):
        ...
        if target is not None and mask is not None:
            return F.rotate(img, angle, self.resample, self.expand, self.center), \
                   F.rotate(target, angle, self.resample_tg, self.expand, self.center), \
                   F.rotate(mask, angle, self.resample_mask, self.expand, self.center)
        return F.rotate(img, angle, self.resample, self.expand, self.center)
...
class RandomAffine(object):
    ...
    def __init__(self, degrees, translate=None, scale=None, shear=None, resample=False, resample_tg=False, resample_mask = False, fillcolor=0):
...
class Grayscale(object):
    ...
    def __call__(self, img, target=None, mask=None):
        ...
        if target is not None and mask is not None:
            return F.to_grayscale(img, num_output_channels=self.num_output_channels), target, mask
    ...
    def __call__(self, img, target=None, mask=None):
        ...
        if random.random() < self.p:
            if target is not None and mask is None:
                ...
                return F.to_grayscale(img, num_output_channels=num_output_channels), target, mask
        if target is not None:
            return img, target
        return img
...
class myRandomDefocusBlur(object):
    """Rotate the image by angle.

    Args:
        radius (sequence or float or int): Range of degrees to select from.
            If degrees is a number instead of sequence like (min, max), the range of degrees
            will be (-degrees, +degrees).
        length (sequence or float or int): Range of degrees to select from.
            If degrees is a number instead of sequence like (min, max), the range of degrees
            will be (-degrees, +degrees).
    """
    ...
    def __init__(self, radius):
        if isinstance(radius, numbers.Number):
            if radius < 0:
                raise ValueError("If radius is a single number, it must be non-negative.")
            self.radius = (0, radius)
        else:
            if len(radius) != 2:
                raise ValueError("If length is a sequence, it must be of len 2.")
            if radius[0] < 0:
                raise ValueError("If length is a sequence, length[0] must be non-negative.")
            if radius[1] < 0:
                raise ValueError("If length is a sequence, length[1] must be non-negative.")
            self.radius = radius

    @staticmethod
    def get_params(radius):
        """Get parameters for ``defocus_blur`` for a random blurring.

        Returns:
            sequence: params to be passed to ``rotate`` for random rotation.
        """
        kernel_radius = random.uniform(radius[0], radius[1])
    ...
    def __repr__(self):
        return self.__class__.__name__ + '(p={})'.format(self.radius)
